...
PHOSPHO_RESIDUES = {"SEP", "TPO", "PTR"}
PHOSPHATE_OXYGEN_ALIASES = (
    {"O1P", "OP1"},
    {"O2P", "OP2"},
    {"O3P", "OP3"},
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="sequential",
        help=(
            "Scan each site on the accumulating grafted structure or independently "
            "on the original input structure."
        ),
    )
    parser.add_argument(
        "--site-order",
    ...
    parser.add_argument(
        ...
        default="skip",
        help=(
            "How to handle a requested site already encoded as SEP/TPO/PTR. "
            "Complete phosphoresidues are skipped by default; incomplete ones always fail."
        ),
    )
    ...
    parser.add_argument("--n-poses", type=int, default=3, choices=[1, 2, 3],
                        help="Number of independently minimized ranked poses to write (default: 3)")
...
def _pose_output_path(base_path: str, rank: int) -> str:
    ...
    path = Path(base_path)
    return str(path.parent / f"{path.stem}_PF{rank}{path.suffix}")
...
def find_residue(model, site: SiteSpec):
    ...
    for chain_id in candidate_chains:
        ...
        # Modified amino acids are frequently stored as HETATM records, for
        # which Bio.PDB uses a non-blank hetero field in the residue id.
        matches = [
            residue for residue in chain
            if residue.id[1] == site.resseq and residue.id[2] == site.icode
        ]
        supported = [
            residue for residue in matches
            if residue.get_resname().strip().upper() in set(PHOSPHO_MAP) | PHOSPHO_RESIDUES
        ]
        if len(supported) == 1:
            return supported[0]
        if len(supported) > 1:
            names = [residue.get_resname().strip().upper() for residue in supported]
            raise GraftingError(
                f"Ambiguous residue for site {chain_id}:{site.resseq}{site.icode.strip() or ''}: {names}"
            )
        if len(matches) == 1:
            return matches[0]
...
def phosphoresidue_completeness(residue) -> Tuple[bool, List[str]]:
    """Return whether an existing SEP/TPO/PTR has a complete PO3 group."""
    atom_names = {atom.get_name().strip().upper() for atom in residue.get_unpacked_list()}
    missing: List[str] = []
    if "P" not in atom_names:
        missing.append("P")
    for aliases in PHOSPHATE_OXYGEN_ALIASES:
        if atom_names.isdisjoint(aliases):
...
def graft_one_site(
    ...
    # A grafted residue represents one concrete conformer. Replace Bio.PDB
    # DisorderedAtom containers with the selected conformer and normalise its
    # altloc label so that PDB and mmCIF output remain unambiguous.
    for disordered_atom in [
        child for child in list(residue.get_list())
        if isinstance(child, DisorderedAtom)
    ]:
        child_altlocs = [atom.get_altloc() for atom in disordered_atom.disordered_get_list()]
        if selected_altloc and selected_altloc in child_altlocs:
            disordered_atom.disordered_select(selected_altloc)
        selected_atom = disordered_atom.selected_child
        replacement = Atom(
            name=selected_atom.get_name(),
            coord=selected_atom.coord.copy(),
            bfactor=selected_atom.get_bfactor(),
            occupancy=1.0,
            altloc=" ",
            fullname=selected_atom.get_fullname(),
            serial_number=selected_atom.get_serial_number(),
            element=selected_atom.element,
        )
        residue.detach_child(disordered_atom.get_id())
        residue.add(replacement)
...
def run_grafting(
    ...
    for model_index in selected_model_indices:
        ...
        preexisting_phospho_ids = {
            residue.full_id for residue in model.get_residues()
            if residue.get_resname().strip().upper() in PHOSPHO_RESIDUES
        }
        ...
        # Preflight every requested site before changing any coordinates.
        preflight_records: List[Dict[str, object]] = []
        seen_site_keys: Set[Tuple[str, int, str]] = set()
        for site in sites_sorted:
            ...
            try:
                ...
                site_key = (chain_id, site.resseq, site.icode.strip())
                record["residue"] = residue
                record["site_key"] = site_key
                original_resname = residue.get_resname().strip().upper()
                record["original_resname"] = original_resname

                if site_key in seen_site_keys:
                    record["preflight_status"] = "DUPLICATE_REQUEST"
                    record["message"] = "The same chain/residue site was requested more than once"
                elif original_resname in PHOSPHO_MAP:
                    seen_site_keys.add(site_key)
                elif original_resname in PHOSPHO_RESIDUES:
                    seen_site_keys.add(site_key)
                    complete, missing_atoms = phosphoresidue_completeness(residue)
                    if not complete:
                        record["preflight_status"] = "INCOMPLETE_PHOSPHORESIDUE"
                        record["message"] = (
                            f"Existing {original_resname} is missing phosphate atoms: "
                            + ", ".join(missing_atoms)
                        )
                    elif existing_site_policy == "error":
                        record["preflight_status"] = "ALREADY_PHOSPHORYLATED_ERROR"
                        record["message"] = (
                            f"Site is already encoded as complete {original_resname}; "
                            "existing-site policy is error"
                        )
                    else:
                        record["preflight_status"] = "ALREADY_PHOSPHORYLATED"
                        record["message"] = (
                            f"Site is already encoded as complete {original_resname}; left unchanged"
                        )
                else:
                    seen_site_keys.add(site_key)
                    record["preflight_status"] = "UNSUPPORTED_RESIDUE"
                    record["message"] = (
                        f"Residue is {original_resname}, not SER/THR/TYR or SEP/TPO/PTR"
                    )
            except Exception as exc:
                record["preflight_status"] = "SITE_NOT_FOUND"
                record["message"] = str(exc)
            preflight_records.append(record)

        print(f"Site preflight (model {model_index}):")
        for record in preflight_records:
            chain_id, resseq, icode = record["site_key"]
            label = f"{chain_id or '?'}:{resseq}{icode}"
            resname = str(record["original_resname"] or "unknown")
            print(f"  {label}  {resname}  {record['preflight_status']}")

        fatal_preflight = [
            record for record in preflight_records
            if record["preflight_status"] not in {"ELIGIBLE", "ALREADY_PHOSPHORYLATED"}
        ]
        if fatal_preflight:
            preflight_path = Path(output_structure).with_suffix(".preflight.tsv")
            preflight_path.parent.mkdir(parents=True, exist_ok=True)
            with preflight_path.open("w", newline="", encoding="utf-8") as handle:
                writer = csv.DictWriter(
                    handle,
                    fieldnames=[
                        "model_index", "chain_id", "resseq", "icode",
                        "original_resname", "preflight_status", "message",
                    ],
                    delimiter="\t",
                )
                writer.writeheader()
                for record in preflight_records:
                    ...
                    writer.writerow({
                        ...
                        "resseq": resseq,
                        "icode": icode,
                        "original_resname": record["original_resname"],
                        "preflight_status": record["preflight_status"],
                        "message": record["message"],
                    })
            failed_labels = ", ".join(
                f"{record['site_key'][0] or '?'}:{record['site_key'][1]}"
                f"{record['site_key'][2]} ({record['preflight_status']})"
                for record in fatal_preflight
            )
            raise GraftingError(
                f"Site preflight failed before grafting: {failed_labels}. "
                f"Details: {preflight_path}"
            )

        # Phase 1: graft every eligible site and prescan it either on the
        # accumulating model or independently on the original input model.
        # No site is minimized until the complete scan phase has finished.
        site_prescan_cache: Dict[Tuple[str, int, str], Dict[str, object]] = {}
        site_residue_cache: Dict[Tuple[str, int, str], object] = {}

        for preflight in preflight_records:
            site = preflight["site"]
            site_key = preflight["site_key"]
            row: Dict[str, object] = {
                "model_index": model_index,
                ...
                "existing_site_policy": existing_site_policy,
                "applied_initial_rotations": "",
            }
            report_rows.append(row)
            preflight_status = str(preflight["preflight_status"])
            residue = preflight.get("residue")
            original_resname = str(preflight.get("original_resname", ""))
            row["original_resname"] = original_resname
            row["new_resname"] = original_resname

            if preflight_status == "ALREADY_PHOSPHORYLATED":
                row["status"] = "SKIPPED"
                row["selected_altloc"] = get_residue_altloc_label(residue, altloc_mode) or ""
                continue
            if preflight_status != "ELIGIBLE" or residue is None:
                row["status"] = "FAILED"
                continue
            ...
            try:
                ...
                if prescan_rotamer and relax_mode == "openmm_local":
                    if scan_context == "independent":
                        prescan_info = independent_prescan_site(
                            ...
                            protonation=protonation,
                            plddt_mean=plddt_mean,
                            prescan_step_deg=prescan_step_deg,
                            prescan_contact_weight=prescan_contact_weight,
                            prescan_basic_weight=prescan_basic_weight,
                            prescan_hbond_weight=prescan_hbond_weight,
                            prescan_polar_clash_scale=prescan_polar_clash_scale,
                            prescan_pack_weight=prescan_pack_weight,
                            prescan_geom_prior_weight=prescan_geom_prior_weight,
                            prescan_tpo_torsion_sigma=prescan_tpo_torsion_sigma,
                            prescan_keep_kabsch_if_good=prescan_keep_kabsch_if_good,
                        )
                        candidates = prescan_info.get("prescan_top_k_candidates", [])
                        if isinstance(candidates, list) and candidates:
                            apply_prescan_candidate_coords(residue, candidates[0])
                    else:
                        prescan_info = prescan_phospho_rotamer(
                            structure,
                            residue,
                        ...
                        prescan_info["prescan_context"] = (
                            f"sequential_grafted_model_premin_{site_order}"
                        )
        ...
        for pose_rank in range(actual_n_poses):
            ...
            if relax_mode == "openmm_local" and minimization_coupling == "joint":
                ...
                if joint_residues:
                    ...
                    if flex_neighbor_radius > 0:
                        ...
                        joint_flex_neighbors = [
                            neighbour for neighbour in joint_flex_neighbors
                            if neighbour.full_id not in preexisting_phospho_ids
                        ]
            ...
            for residue in modified_residues_for_model:
                ...
                if relax_mode == "openmm_local":
                    ...
                    if flex_neighbor_radius > 0:
                        ...
                        flex_neighbors = [
                            neighbour for neighbour in flex_neighbors
                            if neighbour.full_id not in preexisting_phospho_ids
                        ]
            ...
            pose_path = _pose_output_path(output_structure, pose_rank + 1)
    ...
    fieldnames = [
        ...
        "original_resname", "new_resname", "status", "preflight_status", "message",
        ...
        "prescan_context", "scan_context", "site_order", "minimization_coupling",
        "existing_site_policy", "applied_initial_rotations",
    ...
    for pose_path, pose_rows in zip(pose_output_paths, pose_report_rows_all):
        pose_report = (
            report_path
            if actual_n_poses == 1 and report_path
            else str(Path(pose_path).with_suffix(".report.tsv"))
        )
    ...
    import json as _json
    frames_path = str(Path(output_structure).with_suffix(".prescan_frames.json"))
    frames_data = {}
    ...
    print(f"Generated {len(pose_output_paths)} ranked pose output(s).")
...
if __name__ == "__main__":
    ...
    run_grafting(
        ...
        existing_site_policy=args.existing_site_policy,
    )
