...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--scan-context", choices=["sequential", "independent"], default="sequential")
    p.add_argument("--site-order", choices=["n_to_c", "c_to_n", "input"], default="n_to_c")
    p.add_argument("--minimization-coupling", choices=["sequential", "joint"], default="sequential")
    return p.parse_args()
...
def find_residue(model, chain_id: Optional[str], resseq: int, icode: str):
    ...
    matches = [
        ...
        if residue.id[1] == resseq and (residue.id[2] or " ") == (icode or " ")
    ]
    preferred = [
        residue for residue in matches
        if residue.get_resname().strip().upper() in {"SER", "THR", "TYR", "SEP", "TPO", "PTR"}
    ]
    if len(preferred) == 1:
        return preferred[0]
    if len(preferred) > 1:
        names = [residue.get_resname().strip().upper() for residue in preferred]
        raise KeyError(f"Ambiguous residue at {chain.id}:{resseq}{icode.strip()}: {names}")
    if len(matches) == 1:
        return matches[0]
...
def overall_interpretation_label(
    ...
    if prescan_context == "independent_unmodified_model":
        ...
        prefix = ""
    if not multi_site:
        mode_text = "after local minimization"
    elif minimization_coupling == "joint":
        mode_text = "after joint multi-site minimization"
    else:
        mode_text = "after sequential site-wise minimization"
    if low_conf:
...
def write_reports(prefix: str, records: Sequence[Dict]) -> Tuple[Path, Path]:
    ...
    flat_fields = [
        ...
        "graft_status", "graft_message", "graft_mode", "scan_context", "site_order", "minimization_coupling",
        "preflight_status", "prescan_context", "applied_initial_rotations", "optimization_strategy",
...
def main():
    ...
    for token in args.sites:
        ...
        rec = {
            ...
            "graft_status": graft_info.get("graft_status", ""),
            "graft_message": graft_info.get("graft_message", ""),
            "graft_mode": "multi_site" if len(args.sites) > 1 else "single_site",
            "scan_context": args.scan_context,
            "site_order": args.site_order,
            "minimization_coupling": args.minimization_coupling,
            "preflight_status": graft_info.get("preflight_status", ""),
            "prescan_context": graft_info.get("prescan_context", ""),
            "applied_initial_rotations": graft_info.get("applied_initial_rotations", ""),
            "optimization_strategy": (
                "existing_phosphoresidue_left_unchanged"
                if graft_info.get("preflight_status") == "ALREADY_PHOSPHORYLATED"
                else (
                    f"{args.scan_context}_prescan_{args.site_order}_then_"
                    f"{args.minimization_coupling}_minimization"
                )
            ),
        ...
        if rec.get("preflight_status") == "ALREADY_PHOSPHORYLATED":
            rec["overall_interpretation"] = (
                "Existing complete phosphoresidue was detected and left unchanged; "
                "reported measurements describe the input conformation, not a new graft prediction"
            )
        else:
